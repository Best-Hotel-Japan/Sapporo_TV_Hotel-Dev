...
setTimeout(function(){
    ...
    .then(text => {
        ...
        Hotel_information: {                            // HnameTable酒店资料数组初始化
            ...
            HnameTable["未知酒店"]=["未知酒店名","未知Cid","未知","未知Jid","en",[""]];
        ...
        Hotel_RoomType_information: {                   // 各酒店房型资料数组初始化
            ...
            HnameTable["旅亭花"][5]["日式房-带露天"]=["507755385","04禁，02"];
            HnameTable["汤本"][5]["日式房"]=["569488243","07禁，01"];
            HnameTable["汤本"][5]["和洋式房"]=["569488657","04禁，05"];
            HnameTable["汤本"][5]["豪华日式房"]=["569488657","04禁，05"];
...
/房型\/间数\/床型\s+[\w：]{0,3}([-\u4E00-\u9FFF]+).+([\u4e00-\u9fa5]早|早\+晚餐).+\s+(\d+)\s+间\s+房型ID.+ (\d+) .+\s+入离日期\s+(20\d\d\W\d\d\W\d\d).+(20\d\d\W\d\d\W\d\d).+(\d+)\s*晚/;
        ...
        beside_content_display_block: {                 // 根据订单类型，弹出对话框
            ...
            if ( ctripOrderType.match(/修改/) ){
                ...
                myWindow.document.write("<h3>　您辛苦啦！~</h3><h3>　这是一个修改单！一定要先查清楚此单之前所有的关联单号，是否在JTB上已經預訂了房间。</h3><h3>　关联单号可能不止一个！</h3>");
                myWindow.focus();
            }

            if ( ctripOrderType.match(/取消/) ){
                myWindow=window.open('', '', 'width=325, height=240, top=185, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
                myWindow.document.write("<h3>　您辛苦啦！~</h3><h3>　这是一个取消单！一定要先看清楚此单的原始訂單通知时间。</h3><h3>　JTB上的取消操作要做记录！</h3><h3>　最后记得把集計表涂红。</h3>");
                myWindow.focus();
            ...
            if ( ctripOrderType.match(/无效/) ){
                myWindow=window.open('', '', 'width=325, height=240, top=185, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
                myWindow.document.write("<h3>　您辛苦啦！~</h3><h3>　这是一个无效訂單！</h3><h3>　不应该打开接单提示边栏。</h3><h3>　搜索一下关联訂單和相关邮件吧。</h3>");
                myWindow.focus();
                setTimeout(function(){ window.close(); }, 700);
            }   
...
/  ([^w]{1,5})\s+\w{6}-01\s+.+\s+■.+■(男性\w+名).+料金\s[￥\\]([\d,*]+)\s.*■取消料.+[2|に][0|て][\d|確][\d|認][\/|し][\d|て][\d|く][\/|だ][\d|さ][\d|い]\s+＜日程情報.+\s+(20\d\d\/\d\d\/\d\d)\s\sA\)([^ ]+)\s\s([^ ]+.*)\s\s([\w]{3})\s\s([^ ]+\s+[^ ]+).+..[待済]\s{2,3}(\w{5}H)-01/g;
                    ...
                    for ( var j = 1 ; j < indexT+1 ; j++ ){
                        ...
                        if ( roomFee_list[j] == "***" ){ roomFee_list[j] = "--"; }
                        ...
                        +'\t\t'+ repo4+"月" +'\t'+ repo5 +'\t'+ ctripOrderRoomtype +'\t\t'+ ctripOrderEachRoom +'\t\t'+ ctripOrderRooms+" 间" +'\t'+ "每间 "+DailyPrice_list[j] +'\t'+ "原价 "+roomFee_list[j]
                ...
                } else if ( reserv_Order == true ){        // EBK后台操作提示
                    myWindow=window.open('', '', 'width=500, height=320, top=280, left=600, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
                    myWindow.document.write("<h3>　<span style=\"background-color:#32cd32;\"><font color=\"#fffff0\">&#8201保留房&#8201</font></span> 訂單！</h3><h3>　請依據剛才記錄的JTB房型的<font color=\"#CC0033\">残室</font>数字，先回到携程訂單頁面，完成設定：</h3><h3>　<font color=\"#CC0033\">現在残室 >= ３</font></br>　房型ID、點選日期、立即确认房量、余量等于１</h3><h3>　<font color=\"#CC0033\">現在残室 = ０</font></br>　房型ID、點選日期、关房</h3>");
                    myWindow.focus();    
                    myWindow.onclick = function(){
                        myWindow.close();
                    }                    
        ...
        $('#btn_JTB_confirmed').on('click', function () {
            ...
            $('#btn_JTB_confirmed').delay(300).queue(function(){
                window.close();
            });
        ...
        $('#txt_hotel').on('click', function () {
            ...
            .then(() => {
                ...
                if ( reserv_Order == true ){
                    myWindow=window.open('', '', 'width=325, height=240, top=185, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
                    myWindow.document.write("<h3>　您辛苦啦！~</h3><h3>　这是一个 <span style=\"background-color:#32cd32;\"><font color=\"#fffff0\">&#8201保留房&#8201</font></span> 訂單！</h3><h3>　請在検索結果中記錄JTB房型的<font color=\"#CC0033\">残室</font>数字</h3><h3>( 若真的使用押房接单，需要先清查一下相关日期保留房的间数 )</h3>");
                    myWindow.focus();    
                    myWindow.onclick = function(){
                        myWindow.close();
                    }                    
...
$('#faceImg').on('click', function () {
    ...
    .then(() => {
        ...
        if ( gName1 == "" ){
            ...
            myWindow.document.write("<br/><h2>　小伙伴们辛苦啦！~</h2><h2>　　<span style=\"background-color:#31cccc;\"><font color=\"#cc317e\">&#8201去欧罗巴！靠我们自己&#8201</font></span> </h2><br/><h3>　<font color=\"#0c0c33\">本视窗特别鸣谢：</font></h3><br/><h3>　&#9825 原案策划 &#9825　北海道・旭</h3><h3>　&#9825 界面设计 &#9825　NTD-coding猿</h3><h3>　&#9825 开发管理 &#9825　爱大蒜SAMA</h3><h3>　&#9825 产品主试 &#9825　有澤さん</h3><h3>　&#9825 产品副试 &#9825　ホテルKk</h3><h3>　&#9825 黏合创艺 &#9825　Rick</h3><br/><a style=\"font-size:12px;font-family:Meiryo;font-style:oblique;color:rgb(100, 100, 248)\">　　　　　　　　　　　　　　　　　　　　　♪ 未完待续 ♫</a>");
            myWindow.focus();    
            myWindow.onclick = function(){
                myWindow.close();
            }                    
