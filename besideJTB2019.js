...
/房型\/间数\/床型\s+[\w：]{0,3}([-\u4E00-\u9FFF]+).+([\u4e00-\u9fa5]早|早\+晚餐).+\s+(\d+)\s+间\s+房型ID.+ (\d+) .+\s+入离日期\s+(20\d\d\W\d\d\W\d\d).+(20\d\d\W\d\d\W\d\d).+(\d+)\s*晚/;
        ...
        report_summary_editing_block: {                 // 生成不带JTB信息的记录, 传给剪贴板 和 云端
        ...
        modal_content_display_block: {                  // 编辑 MS 报告, 处理按钮相应
            ...
            case "1":
                    ...
                    case "5":
                            ...
                            case "3": MS_report = "番" + ctripOrder_1Ref + "　代表者：" + gName[1] + "　" + gName[2] + "　" + gName[3]; break;
                            case "4": MS_report = "番" + ctripOrder_1Ref + "　同行者：" + gName[2] + "　" + gName[3] + "　" + gName[4]; break;
                            case "5": MS_report = "番" + ctripOrder_1Ref + "　同行者：" + gName[2] + "　" + gName[3] + "　" + gName[4] + "　" + gName[5]; break;
                            } break;
                    } break;
                ...
                setTimeout(function(){
                    ...
                    .then(() => {
                        ...
                        if ( MS_report_finish == 0 ){
                            myWindow=window.open('', '', 'width=325, height=240, top=185, left=500, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
                            myWindow.document.write("<h3>　您辛苦啦！~</h3><h3>　这个訂單预订房间数太多了！也有可能是通知的姓名不规范，无法自动处理。</h3><h3>　請您自己手动编辑MS报告！</h3>");
                            myWindow.focus();
                        }
                    })
                    .catch(() => {
        ...
        $('#btn_JTB_confirmed').on('click', function () {
            ...
            .then(() => {
                ...
                if ( reserv_Order_Tips == 1 ){        // 关于保留房订单的EBK后台操作提示
                    ...
                    myWindow=window.open('', '提示', 'width=505, height=320, top=240, left=600, toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no');
